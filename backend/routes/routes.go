...
func SetupRoutes(app *fiber.App) {
	...
	app.Get("/api/player/profile/:id", handler.GetProfilePageStuff).Name("Get Matches from ID")
	app.Get("/api/player/wins/:id", handler.GetWinsStruct).Name("Get Wins Array")
	app.Get("/api/player/games", handler.GamesAll).Name("Get Games")
